                ...
                <form method="GET" action="{{ route('event.index') }}" id="filterForm">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
                            ...
                            <div x-show="open" x-cloak class="pb-4 space-y-2 text-sm text-slate-500">
                                @foreach ($locations as $location)
                                    <label class="flex items-center space-x-3 cursor-pointer group">
                                        ...
                                        <span class="group-hover:text-slate-900 transition-colors">{{ $location }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div x-data="{ open: {{ request('price') && request('price') !== 'free' ? 'true' : 'false' }} }" class="border-b border-gray-100">
                            ...
                            <div x-show="open" x-cloak class="pb-4 space-y-2 text-sm text-slate-500">
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    <input type="radio" name="price" value="paid" class="rounded-full text-blue-600 focus:ring-blue-500"
                                        {{ request('price') === 'paid' ? 'checked' : '' }}
                                        onchange="document.getElementById('filterForm').submit()">
                                    <span class="group-hover:text-slate-900 transition-colors">Berbayar</span>
                                </label>
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    <input type="radio" name="price" value="free" class="rounded-full text-blue-600 focus:ring-blue-500"
                                ...
                                @foreach ($categories as $category)
                                    <label class="flex items-center space-x-3 cursor-pointer group">
                                        <input type="radio" name="category" value="{{ $category->slug }}"
                                            ...
                                            onchange="document.getElementById('filterForm').submit()">
                                        <span class="group-hover:text-slate-900 transition-colors">{{ $category->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                                ...
                                @foreach ([
                                    ...
                                    <label class="flex items-center space-x-3 cursor-pointer group">
                                        <input type="radio" name="time" value="{{ $value }}"
                                            ...
                                            onchange="document.getElementById('filterForm').submit()">
                                        <span class="group-hover:text-slate-900 transition-colors">{{ $label }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <form method="GET" action="{{ route('event.index') }}" id="sortForm">
                            ...
                            <div class="relative">
                                <select name="sort" onchange="this.form.submit()"
                                    ...
                                    <option value="" {{ !request('sort') ? 'selected' : '' }}>Tampilkan Semua</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    @forelse ($events as $event)
                        ...
                        <a href="{{ route('event.show', $event->id) }}"
                            ...
                            <div class="relative h-44 overflow-hidden">
                                ...
                                {{-- Kategori Badge --}}
                                <div class="absolute top-3 left-3 bg-gradient-to-r from-blue-500 to-indigo-500 px-2.5 py-1 rounded-full text-[9px] font-bold text-white uppercase shadow-lg">
                            ...
                            <div class="p-4 flex-grow flex flex-col justify-between">
                                <div class="space-y-1.5">
                                    <h4 class="font-bold text-slate-900 text-base leading-tight line-clamp-2 group-hover:text-blue-600 transition-colors">
                                        {{ $event->title }}
                                    </h4>
                                ...
                                <div class="pt-3 mt-3 border-t border-slate-100 flex items-center space-x-2.5">
                                    <div class="w-6 h-6 rounded-full bg-blue-50 flex items-center justify-center text-[10px] font-bold text-blue-600 border border-blue-100 shadow-sm">
                                        {{ strtoupper(substr($event->creator->name ?? 'U', 0, 1)) }}
                                    </div>
                    ...
                    @empty
                        ...
                        <div class="col-span-full text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                            ...
                            <a href="{{ route('event.index') }}" class="mt-6 inline-flex items-center text-sm font-bold text-blue-600 hover:text-blue-700">
