<aside :class="sidebarOpen ? 'w-60' : 'w-16'"
    ...
    <nav class="flex-grow flex flex-col px-0 overflow-y-auto custom-scrollbar">
        ...
        <a href="{{ route('creator.dashboard') }}"
            class="flex items-center gap-3.5 px-5 py-3 transition whitespace-nowrap {{ request()->routeIs('creator.dashboard') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/5 group' }}">
        ...
        <a href="{{ route('creator.eventsaya.index') }}"
            class="flex items-center gap-3.5 px-5 py-3 transition whitespace-nowrap {{ request()->routeIs('creator.eventsaya.index', 'creator.eventsaya.stats') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/5 group' }}">
        ...
        <a href="{{ route('creator.rekening.index') }}"
            class="flex items-center gap-3.5 px-5 py-3 transition whitespace-nowrap {{ request()->routeIs('creator.rekening.index', 'creator.rekening.create') ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-white/5 group' }}">
            <i class="fas fa-wallet text-base group-hover:text-blue-400 w-5 text-center"></i>
            <span x-show="sidebarOpen" x-transition.opacity class="text-sm font-bold">Rekening</span>
        </a>

        <div class="mt-6 px-5 mb-1.5">
            <span x-show="sidebarOpen" x-transition.opacity
                class="text-[9px] font-bold uppercase tracking-[0.15em] text-slate-500">Mode User</span>
            <hr x-show="!sidebarOpen" class="border-white/10">
        </div>

        <a href="{{ route('home') }}"
            class="flex items-center gap-3.5 px-5 py-3 text-slate-400 hover:bg-white/5 transition group whitespace-nowrap">
            <i class="fas fa-sync-alt text-base group-hover:text-blue-400 w-5 text-center"></i>
            <span x-show="sidebarOpen" x-transition.opacity class="text-sm font-medium">Beralih Ke Pembeli</span>
        </a>
    </nav>
