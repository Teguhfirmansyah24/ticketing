<x-admin-layout>
    ...
    <div class="p-8 bg-[#fbfcfd] min-h-screen font-sans text-slate-700">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="glass-card p-6 border-t-4 border-t-rose-500">
                <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Okupansi</p>
                <h3 class="text-2xl font-black text-slate-900 mt-2" id="statAvgOccupancy">0%</h3>
            </div>
        </div>
                ...
                <table class="w-full text-left custom-table">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-4">Progress Kapasitas</th>
    ...
    <script>
        ...
        /**
         * Membersihkan input agar menjadi angka yang bisa dikalkulasi
         */
        function cleanNumber(val) {
            if (typeof val === 'number') return val;
            if (!val) return 0;
            // Menghapus Rp, titik ribuan, atau karakter non-angka lainnya
    ...
    if (!data || data.length === 0) {
        // ... (logika jika data kosong tetap sama)
        return;
    }
    ...
    data.forEach(event => {
        // --- PERBAIKAN DI SINI ---
        // Kita cek apakah namanya 'revenue' atau 'total_amount' atau 'total_harga'
        const revenue = cleanNumber(event.revenue || event.total_amount || event.total_harga || 0);
        const sold = cleanNumber(event.sold_count || event.total_sold || event.qty || 0);
        const capacity = cleanNumber(event.total_capacity || event.capacity || 0);
        // -------------------------
        ...
        const name = event.name || event.title || 'Tanpa Nama';
        const catName = event.category_name || 'Tanpa Kategori';
        const occ = capacity > 0 ? Math.min(100, Math.round((sold / capacity) * 100)) : 0;
        ...
        tableBody.innerHTML += `
            <tr class="hover:bg-slate-50/50 transition-colors">
                ...
                <td class="px-6 py-4 text-center font-semibold text-slate-700 text-sm">${sold.toLocaleString('id-ID')}</td>
                <td class="px-6 py-4 w-64">
                    <div class="flex items-center gap-3">
                        <div class="flex-grow bg-slate-100 rounded-full h-2 overflow-hidden">
                            <div class="bg-blue-500 h-full rounded-full" style="width: ${occ}%"></div>
                        </div>
                        <span class="text-[10px] font-bold text-slate-500">${occ}%</span>
                    </div>
                </td>
                <td class="px-6 py-4 text-right font-bold text-slate-900 text-sm">${formatRupiah(revenue)}</td>
            </tr>
        `;
    });
    ...
    document.getElementById('statTickets').innerText = totalSold.toLocaleString('id-ID');
    document.getElementById('statEvents').innerText = data.length;
    document.getElementById('statAvgOccupancy').innerText = (totalCap > 0 ? Math.round((totalSold / totalCap) * 100) : 0) + '%';
    ...
    renderCharts(data);
            ...
            cChart = new Chart(catCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#3b82f6', '#6366f1', '#0ea5e9', '#f43f5e', '#8b5cf6'],
                        borderWidth: 0
                    }]
                },
                options: {
        ...
        document.getElementById('applyFilterBtn').onclick = function() {
            ...
            const filtered = originalEventData.filter(e => {
                const date = e.event_date; // Pastikan field event_date ada di data anda
