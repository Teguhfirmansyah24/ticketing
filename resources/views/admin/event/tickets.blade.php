...
<div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
    ...
    <form action="{{ route('admin.tickets.store', $event->id) }}" method="POST" class="grid grid-cols-4 gap-4 mb-8">
        ...
        <input type="number" name="stock" placeholder="Jumlah Stok" class="rounded-xl border-gray-200 text-sm">
    ...
    <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="text-sm">
                ...
                <td class="px-4 py-4">
                    <span class="px-2 py-1 {{ $ticket->stock < 10 ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600' }} rounded-lg font-bold">
                        {{ $ticket->stock }}
                    </span>
                </td>
