                        ...
                        @forelse($orders as $order)
                            <tr class="hover:bg-sky-50/30 transition-colors">
                                ...
                                <td class="px-6 py-5">
                                    ...
                                    <div
                                        class="text-[10px] px-2 py-0.5 bg-sky-50 text-sky-600 rounded-md inline-block mt-1 font-medium">
                                        {{ $order->event?->title ?? 'Event Tidak Ditemukan' }}
                                    </div>
                                </td>

                                <td class="px-6 py-5">
                                    <div class="text-sm font-black text-sky-900">Rp
                                        {{ number_format($order->total_amount, 0, ',', '.') }}</div>
                                    <div class="text-[10px] text-emerald-500 font-bold tracking-tight uppercase italic">
                                        Midtrans / Gateway</div>
                                </td>

                                <td class="px-6 py-5">
                                    @if ($order->status == 'approved' || $order->status == 'success')
                                        <span
                                            class="px-3 py-1.5 rounded-xl bg-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span
                                                class="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-2 animate-pulse"></span>
                                            Approved
                                        </span>
                                    @elseif($order->status == 'pending')
                                        <span
                                            class="px-3 py-1.5 rounded-xl bg-amber-100 text-amber-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span
                                    ...
                                    @elseif($order->status == 'expired')
                                        <span
                                            class="px-3 py-1.5 rounded-xl bg-red-100 text-gray-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-gray-500 rounded-full mr-2"></span> Expired
                                        </span>
                                    @elseif($order->status == 'cancelled')
                                        <span
                                            class="px-3 py-1.5 rounded-xl bg-gray-100 text-red-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-red-500 rounded-full mr-2"></span> Cancelled
                                        </span>
                                    @else
                                        <span
                                            class="px-3 py-1.5 rounded-xl bg-red-100 text-red-600 text-[10px] font-black uppercase tracking-wider">
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @if ($order->status == 'pending')
                                            ...
                                            <form action="{{ route('admin.orders.approve', $order->id) }}"
                                                method="POST" onsubmit="return confirm('Approve pesanan ini?')">
                                                @csrf
                                                <button type="submit"
