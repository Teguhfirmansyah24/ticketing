<x-admin-layout>
    <style>
        @media print {
            /* Force colors and backgrounds to show */
            * {
                -webkit-print-color-adjust: exact !important;
                print-color-adjust: exact !important;
            }

            /* Hide Sidebar, Header, Search, Filters, and Action Buttons */
            nav, aside, header, .no-print, form, .flex.gap-2, .flex.gap-3, th:last-child, td:last-child {
            ...
            /* Status Badges - Force them to show colors */
            .bg-emerald-100 { background-color: #d1fae5 !important; color: #059669 !important; }
            .bg-amber-100 { background-color: #fef3c7 !important; color: #d97706 !important; }
    ...
    <div class="p-6 space-y-8">
        ...
        @if (session('success'))
            <div class="p-4 mb-4 text-sm text-emerald-800 rounded-2xl bg-emerald-50 border border-emerald-100 no-print">
                <span class="font-bold">Berhasil!</span> {{ session('success') }}
            </div>
        @endif

        <div class="bg-white rounded-3xl shadow-xl shadow-sky-100/50 border border-sky-100 overflow-hidden">
            <div class="p-6 border-b border-sky-50 flex flex-wrap gap-4 items-center justify-between bg-sky-50/20 no-print">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-sky-600 text-white">
                            ...
                            <th class="px-6 py-4 font-bold uppercase text-[10px] tracking-[0.2em] text-center no-print">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-sky-50">
                                ...
                                <td class="px-6 py-5">
                                    @if ($order->status == 'approved' || $order->status == 'success')
                                        <span class="px-3 py-1.5 rounded-xl bg-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-2 animate-pulse no-print"></span> Approved
                                        </span>
                                    @elseif($order->status == 'pending')
                                        <span class="px-3 py-1.5 rounded-xl bg-amber-100 text-amber-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-amber-500 rounded-full mr-2 animate-pulse no-print"></span> Pending
                                        </span>
                                    @elseif($order->status == 'expired')
                                        <span class="px-3 py-1.5 rounded-xl bg-red-100 text-gray-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-gray-500 rounded-full mr-2 no-print"></span> Expired
                                        </span>
                                    @elseif($order->status == 'cancelled')
                                        <span class="px-3 py-1.5 rounded-xl bg-gray-100 text-red-600 text-[10px] font-black uppercase tracking-wider flex items-center w-fit">
                                            <span class="w-1.5 h-1.5 bg-red-500 rounded-full mr-2 no-print"></span> Cancelled
                                        </span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @if ($order->status == 'pending')
                                            ...
                                            <form action="{{ route('admin.orders.approve', $order->id) }}" 
                                                method="POST" 
                                                onsubmit="return confirm('Konfirmasi: Apakah Anda yakin ingin menyetujui pesanan ini?')">
                                                @csrf
                                                <button type="submit" 
                                        ...
                                        <form action="{{ route('admin.orders.destroy', $order->id) }}" 
                                            method="POST" 
                                            onsubmit="return confirm('Peringatan: Menghapus data ini akan menghilangkannya secara permanen. Lanjutkan?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sky-400 italic font-bold">Belum ada transaksi masuk.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="p-6 bg-sky-50/20 border-t border-sky-50 no-print">
                {{ $orders->links() }}
            </div>
        </div>
    </div>
